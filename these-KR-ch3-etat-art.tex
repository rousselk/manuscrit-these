...
\flcaption{Fonctionnement du protocole ContikiMAC.
(D'après \cite{coursEnsem})}
\label{FigContikiMAC}
\end{figure}
...
\flcaption{Survenue de <<~collisions cachées~>> entre deux paires
de noeuds menant chacune une transmission distincte, empêchant
la réussite des deux communications.
(D'après \cite{coursEnsem} et \cite{RIMAC})}
...
\mbox{S-CoSenS}. (D'après \cite{TheseBNefzi})}
...
Le mécanisme \nom{T-AAD} consiste à adapter dynamiquement les paramètres
...
en mode \lang{``anycast''} tout récepteur est tenu de renvoyer une trame
...
aux premiers envois de trames en mode \lang{``anycast''}, les transmissions
suivantes de trames de données peuvent se faire en mode \lang{``unicast''}
...
les noeuds mobiles des WSN. (Source~: \cite{ME-ContikiMAC})}
...
\nom{ME-ContikiMAC}. Ce dernier protocole a été conçu pour mieux
...
à communiquer durant son \lang{slot} assigné.
