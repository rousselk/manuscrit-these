...
Pour résoudre ce problème, nous avons proposé une extension de l'API des
...
Une fonctionnalité unique de la pile réseau de Contiki est d'être centrée
autour d'un unique \lang{buffer} destiné à contenir la trame réseau
802.15.4 en cours de traitement (nommé \lang{``packetbuf''} dans la
terminologie Contiki). Ce \lang{packetbuf} constitue ainsi la <<~zone de
...
\item \emph{Le risque de pertes de trames.} Ce \lang{buffer} unique devant
...
cycles''}) du réseau, afin d'implanter une stratégie efficace d'économie
d'énergie pour les noeuds du réseau~--- et la couche \nom{MAC (\lang{Media
Access Control})}~--- qui en théorie se charge uniquement d'ordonnancer
...
\item[4) MAC~:] l'implantation du protocole \lang{Media Access Control},
...
\flcaption{Représentation fonctionnelle de l'architecture de la pile
réseau de Contiki.
...
matérielle de la mémoire (notamment une \nom{MMU}~: unité de gestion
...
\cite{PRriotEnh5} (sans devoir attendre~/ dépendre d'un \lang{watchdog}).
